from lark_channel.core.exception import NoAuthorizationException
...
def verify(config: Config, request: BaseRequest, option: RequestOption) -> None:
    # Endpoint does not require a token
    if len(request.token_types) == 0:
        return

    # When token config is enabled, the token must be passed manually
    ...
    # When token config is disabled, obtain the token from app_id/app_secret
