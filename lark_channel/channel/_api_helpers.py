...
**Internal module — do not import from outside ``lark_channel.channel``.** The
...
Pure async functions taking the underlying ``lark_channel.Client``. Extracted
...
async def default_name_lookup(
    ...
    if not open_ids:
        return {}
    try:
        from lark_channel.api.contact.v3.model.batch_user_request import (
            BatchUserRequest,
        )
...
async def fetch_chat_info(lark_client: Any, chat_id: str) -> Optional[ChatInfo]:
    ...
    """
    try:
        from lark_channel.api.im.v1.model.get_chat_request import GetChatRequest
        ...
        for attr in ("name", "description", "chat_type", "chat_mode", "owner_id", "user_count"):
...
async def fetch_message_raw(
    ...
    message_id: str,
    *,
    card_content_type: Optional[str] = None,
    ...
    try:
        ...
        if card_content_type:
            req.add_query("card_msg_content_type", card_content_type)
        resp = await lark_client.im.v1.message.aget(req)
        code = resp.get("code") if isinstance(resp, dict) else getattr(resp, "code", None)
        if code is not None and code != 0:
            return None
        return _object_to_dict(resp)
    except Exception:
        return None
    ...
    *,
    ...
    try:
        ...
        if message_id:
            ...
            resp = await _get_off_thread(lark_client.im.v1.message_resource, req)
        elif resource_type == "image":
            from lark_channel.api.im.v1.model.get_image_request import GetImageRequest
            req = GetImageRequest.builder().image_key(file_key).build()
            resp = await _get_off_thread(lark_client.im.v1.image, req)
        else:
            from lark_channel.api.im.v1.model.get_file_request import GetFileRequest
            req = GetFileRequest.builder().file_key(file_key).build()
            resp = await _get_off_thread(lark_client.im.v1.file, req)
...
async def mark_read(lark_client: Any, *, message_ids: List[str]) -> Dict[str, Any]:
    try:
        from lark_channel.api.im.v1.model.read_users_message_request import (
            ReadUsersMessageRequest,
        )
        from lark_channel.api.im.v1.model.read_users_message_request_body import (
...
async def _get_off_thread(resource: Any, request: Any) -> Any:
    to_thread = getattr(asyncio, "to_thread", None)
    if to_thread is not None:
        return await to_thread(resource.get, request)
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(None, resource.get, request)
